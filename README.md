...
> Cloudflare-bypassing Playwright browser as MCP server. Fork, connect, scrape.

Real browser automation that doesn't get detected. Ported from [puppeteer-real-browser](https://github.com/ZFC-Digital/puppeteer-real-browser) techniques, wrapped as an MCP server so any AI agent can use it.

## What it does

- Launches a **real Chromium browser** via Playwright
- Applies **anti-detection patches** (webdriver, plugins, languages, mouse events)
- **Bypasses Cloudflare** and other bot detection
- **MCP protocol** — connect from Claude Desktop, Cursor, or any MCP client
...
```bash
echo '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"browser_launch","arguments":{}}}' | playwright-stealth-mcp
```
...
| `browser_snapshot` | Get page text |
| `browser_click` | Click element (human-like) |
| `browser_type` | Type text (human-like) |
| `browser_evaluate` | Execute JavaScript |
| `browser_select` | Select dropdown option |
| `browser_press` | Press keyboard key |
| `browser_wait` | Wait for element/timeout |
...
- ✨ Mouse screen coordinate fix
- ✨ Human-like mouse movement curves
- ✨ Human-like typing with random delays
- ✨ Real Chrome user agent
- ✨ `--disable-blink-features=AutomationControlled`
- ✨ Permission query faking

## Why this exists

Microsoft's [playwright-mcp](https://github.com/microsoft/playwright-mcp) is great but gets detected by Cloudflare. [puppeteer-real-browser](https://github.com/ZFC-Digital/puppeteer-real-browser) bypasses detection but uses Puppeteer. This combines both: Playwright + stealth + MCP.
